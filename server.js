...
async function validateCookies(cookies) {
  ...
  // Remova ou comente as linhas abaixo se quiser reativar a validação
  ...
  /* CÓDIGO DE VALIDAÇÃO ORIGINAL - Descomente para reativar
  const normalizedCookies = normalizeCookies(cookies);
  if (!normalizedCookies) {
    addLog('Normalização de cookies falhou');
    return false;
  ...
  try {
    ...
    addLog('Criando sessão com cookies para validação');
    const session = axios.create({
      withCredentials: true,
      timeout: 30000, // Timeout aumentado para 30 segundos
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        'Cookie': normalizedCookies.map(cookie => `${cookie.name}=${cookie.value}`).join('; ')
      }
    });
    
    // Verificar se os cookies são válidos acessando a página principal
    addLog('Enviando requisição para verificar validade dos cookies...');
    const mainPage = await session.get(`${TRAVIAN_URL}/dorf1.php`);
    
    addLog(`Resposta recebida: status ${mainPage.status}`);
    
    // Verificar se estamos logados
    const isValid = mainPage.data.includes('dorf1') || 
                    mainPage.data.includes('Recursos') || 
                    mainPage.data.includes('stockBarWarehouse') ||
                    mainPage.data.includes('villageNameField');
    
    if (isValid) {
      addLog('Cookies validados com sucesso!');
      // Salvar cookies normalizados
      saveCookies(normalizedCookies);
    } else {
      addLog('Cookies inválidos: não foi possível confirmar login');
    }
    
    return isValid;
  } catch (error) {
    addLog(`Erro ao validar cookies: ${error.message}`);
    if (error.response) {
...
async function runAutomation() {
  ...
  try {
    ...
    // Verificar se os cookies são válidos acessando a página principal
    addLog('Verificando validade dos cookies...');
    try {
      const mainPage = await session.get(`${TRAVIAN_URL}/dorf1.php`);
      ...
      if (!mainPage.data.includes('dorf1') && 
          ...
          !mainPage.data.includes('stockBarWarehouse') &&
          !mainPage.data.includes('villageNameField')) {
        addLog('Cookies inválidos ou expirados. Importe os cookies novamente.');
        return;
      }
      
      addLog('Cookies válidos! Usuário autenticado com sucesso.');
    } catch (error) {
      addLog(`Erro ao verificar cookies: ${error.message}`);
      if (error.response) {
        addLog(`Status do erro: ${error.response.status}`);
      }
      
      // Continuar mesmo com erro na verificação
      addLog('Continuando mesmo com erro na verificação de cookies...');
    }
